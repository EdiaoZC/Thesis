layui.config({
    ...
    $("body").on("click", ".users_edit", function () {
        ...
        var index = layui.layer.open({
            title: "修改成员",
            type: 2,
            content: "editRole.html?id=" + _this.attr("data-id"),
            success: function (layero, index) {
                setTimeout(function () {
                    layui.layer.tips('点击此处返回角色列表', '.layui-layer-setwin .layui-layer-close', {
                        tips: 3
    ...
    function deviceList() {
        function renderDate(data, curr) {
            ...
            if (currData.length != 0) {
                for (var i = 0; i < currData.length; i++) {
                    var permissions = currData[i].permissions;
                    console.log(permissions);
                    var permissionInfo = '';
                    if (permissions.length != 0) {
                        for (var j = 0; j < permissions.length; j++) {
                            permissionInfo += permissions[j].perName;
                            permissionInfo += ',';
                        }
                        permissionInfo = permissionInfo.substring(0, permissionInfo.length);
                    } else {
                        permissionInfo = "暂无数据";
                    }
                    dataHtml += '<tr>'
